...
//     float gray = (fireColor.r + fireColor.g + fireColor.b) / 3.0;
//     fireColor = vec4(gray, gray, gray, fireColor.a);
...
// Use the clamping sample state instead of the wrap sample state to prevent flames wrapping around.
    alphaColor = mix(texture(alphaTextureOne, noiseCoords.xy) , texture(alphaTextureTwo , noiseCoords.xy) , checkVariable);
// Set the alpha blending of the fire to the perturbed and distored alpha texture value.
    fireColor.a = alphaColor.r * 0.3;
