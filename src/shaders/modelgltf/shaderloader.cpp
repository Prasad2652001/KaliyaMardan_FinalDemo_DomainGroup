...
glshaderprogram::glshaderprogram(initializer_list<std::string> shaderList, int version, int profile)
{
	...
	for (std::string shaderPath : shaderList)
	{
		...
		if (!file.is_open())
		{
			PrintLogString("'" + shaderPath + "' file not found.");
		}
		...
		{
			PrintLogString(shaderPath + " extension not mapped or invalid. Try files with the following extensions: [.vert .tesc .tese .geom .frag .comp]");
		}
		...
		if (!compiledStatus)
		{
			...
			PrintLogString(shaderPath + ":" + string(buffer));
		}
	...
	if (!linkedStatus)
	{
		...
		PrintLogString("glsl linker : " + string(buffer));
	}
...
GLint glshaderprogram::getUniformLocation(string uniformName)
{
	...
	{
		PrintLogString("'" + uniformName + "' not an active uniform.");
	}
