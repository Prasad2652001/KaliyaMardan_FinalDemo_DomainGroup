...
struct Vertex_N
{
	...
	vec3 Tangent;
	vec3 Bitangent;
	int BoneIDs[MAX_BONE_INFLUENCE_N];
	float Weights[MAX_BONE_INFLUENCE_N];
};
...
texture loadEmbeddedTextures(const aiMaterial *mat, aiTextureType type, textureTypes typeString, const aiScene *scene)
{
	texture tex;
	aiString str;
	...
	while (!nodeTreeStack.empty())
	{
		...
		for (unsigned int i = 0; i < node->mNumMeshes; i++)
		{
			...
			for (unsigned int i = 0; i < mesh->mNumVertices; i++)
			{
				...
				for (int i = 0; i < MAX_BONE_INFLUENCE_N; i++)
				{
					vertex.BoneIDs[i] = -1;
			...
			glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex_N), (void *)offsetof(Vertex_N, Position));
			glEnableVertexAttribArray(1);
			glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex_N), (void *)offsetof(Vertex_N, Normal));
			glEnableVertexAttribArray(2);
			glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex_N), (void *)offsetof(Vertex_N, TexCoords));
			glEnableVertexAttribArray(3);
			glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex_N), (void *)offsetof(Vertex_N, Tangent));
			glEnableVertexAttribArray(4);
			glVertexAttribPointer(4, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex_N), (void *)offsetof(Vertex_N, Bitangent));
			glEnableVertexAttribArray(5);
			glVertexAttribIPointer(5, 4, GL_INT, sizeof(Vertex_N), (void *)offsetof(Vertex_N, BoneIDs));
			glEnableVertexAttribArray(6);
			glVertexAttribPointer(6, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex_N), (void *)offsetof(Vertex_N, Weights));
...
glmodel::glmodel(string path, unsigned flags, bool isPbr)
{
	...
	if (!scene)
	{
		PrintLogString("Error: Assimp could not load scene for '" + path + "'");
	}
	else if (scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE)
	{
		PrintLogString("Error: Assimp scene incomplete for '" + path + "'");
	}
	else if (!scene->mRootNode)
	{
		PrintLogString("Error: Assimp root node empty for '" + path + "'");
	}
	if (scene->HasMeshes())
	{
	...
	if (scene->HasMaterials())
	{
		for (size_t m = 0; m < scene->mNumMaterials; m++)
		{
			...
			// 		mat->GetTexture(aiTextureType_BASE_COLOR, i, &name_);
			// 		PrintLogString(name_.C_Str());
			// 	}
			// }
			...
			if (isPbr)
			{
				...
				{
					temp.metallic = std::max(temp.specular[0], std::max(temp.specular[1], temp.specular[2]));
				...
				tempTex = loadEmbeddedTextures(mat, aiTextureType_BASE_COLOR, TEX_DIFFUSE, scene);
				if (tempTex.id != 0)
					temp.textures.push_back(tempTex);

				// tempTex = loadPBRTextures(TEX_NORMAL, path.substr(0, path.find_last_of('/')), name.data);
				tempTex = loadEmbeddedTextures(mat, aiTextureType_NORMALS, TEX_NORMAL, scene);
				if (tempTex.id != 0)
					temp.textures.push_back(tempTex);

				// tempTex = loadPBRTextures(TEX_PBR, path.substr(0, path.find_last_of('/')), name.data);
				// tempTex = loadEmbeddedTextures(mat, aiTextureType_DIFFUSE, TEX_DIFFUSE, scene);
				// if (tempTex.id != 0)
				// 	temp.textures.push_back(tempTex);

				// tempTex = loadPBRTextures(TEX_SPECULAR, path.substr(0, path.find_last_of('/')), name.data);
				tempTex = loadEmbeddedTextures(mat, aiTextureType_SPECULAR, TEX_SPECULAR, scene);
				if (tempTex.id != 0)
					temp.textures.push_back(tempTex);

				// tempTex = loadPBRTextures(TEX_EMISSIVE, path.substr(0, path.find_last_of('/')), name.data);
				tempTex = loadEmbeddedTextures(mat, aiTextureType_EMISSIVE, TEX_EMISSIVE, scene);
				if (tempTex.id != 0)
					temp.textures.push_back(tempTex);
			}
			else
