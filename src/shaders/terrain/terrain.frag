...
uniform sampler2D sand_green, grass1_green, grass_green, rock_green, sand_dark, grass1_dark, grass_dark, rock_dark, rockNormal, snow_dark, snow_green;

uniform float u_transitionFactor;

uniform vec4 ufogColor;
uniform float u_fogFalloff;
uniform float uLayeredFogTop;
uniform vec3 u_fogColor;

uniform bool isGodRaysOcclusion;
...
vec4 getTexture(inout vec3 normal, const mat3 TBN) {
	...
	if(WorldPos.y > snowHeight - trans * transMultiplier && WorldPos.y < snowHeight + trans * transMultiplier) {
		...
		grass_t.rgb = mix(grass_t.rgb, texture(snow_green, texCoord * 5.0).rgb * 1.35, gradient);
		grassCoverage = mix(grassCoverage, grassCoverage - 0.12, gradient);
	} else if(WorldPos.y > snowHeight + trans * transMultiplier) {
		grass_t.rgb = texture(snow_green, texCoord * 5.0).rgb * 1.35;
...
void main() {
	...
	// Shadow
		...
		vec4 color = heightColor * vec4((ambient + diffuse * (1.0 - shadow * 0.7)) * vec3(1.0), 1.0);
...
float calculateFogDensity(vec3 WorldPos, vec3 CameraPos, float uFogEnd, float uLayeredFogTop) {
    // Flatten the positions to the xz plane
	vec3 CameraProj = CameraPos;
	CameraProj.y = 0.0;

	vec3 PixelProj = WorldPos;
	PixelProj.y = 0.0;

	float DeltaD = length(CameraProj - PixelProj) / uFogEnd;
	float DeltaY = 0.0;
	float DensityIntegral = 0.0;

	if(CameraPos.y > uLayeredFogTop) {
		if(WorldPos.y < uLayeredFogTop) {
			DeltaY = (uLayeredFogTop - WorldPos.y) / uLayeredFogTop;
			DensityIntegral = DeltaY * DeltaY * 0.5;
		}
	} else {
		if(WorldPos.y < uLayeredFogTop) {
			float DeltaCamera = (uLayeredFogTop - CameraPos.y) / uLayeredFogTop;
			float DensityIntegralCamera = DeltaCamera * DeltaCamera * 0.5;

			float DeltaPixel = (uLayeredFogTop - WorldPos.y) / uLayeredFogTop;
			float DensityIntegralPixel = DeltaPixel * DeltaPixel * 0.5;

			DensityIntegral = abs(DensityIntegralCamera - DensityIntegralPixel);
		} else {
			DeltaY = (uLayeredFogTop - CameraPos.y) / uLayeredFogTop;
			DensityIntegral = DeltaY * DeltaY * 0.5;
		}
	}

	float FogDensity = (DeltaD != 0.0) ? sqrt(1.0 + ((DeltaD / DeltaY) * DeltaD)) * DensityIntegral : 0.0;
