...
public:
    ...
    Terrain(int tileW_)
    {
        ...
        terrainLightDirection = vmath::normalize(vec3(-0.488f, 0.408f, 0.816f));
    ...
    void draw(float isShadow)
    {
        ...
        shad->setVec3("u_fogColor", vec3(0.8f, 0.8f, 0.8f));
        shad->setVec3("u_seed", vec3(93.5972, 39.7979, 24.327));

        shad->setBool("isGodRaysOcclusion", isGodRaysOcclusion);
        ...
        shad->setFloat("uFogEnd", FogEnd);
        shad->setFloat("uLayeredFogTop", LayeredFogTop);
        shad->setVec3("ufogColor", vec3(0.1f, 0.1f, 0.2f));
        shad->setFloat("FogStart", FogStart);

        // Thin try
        shad->setVec3("uSkyColor", vec3(1.0f, 1.0f, 1.0f));
