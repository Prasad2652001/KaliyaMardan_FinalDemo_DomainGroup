...
BOOL loadTextureForFire(GLuint *texture, const char *filename, bool wrap)
{
    ...
    imageData = stbi_load(filename, &width, &height, &bpp, STBI_rgb_alpha);
    if (imageData != NULL)
    {
        ...
        // Load the image data into the texture unit (with RGBA format).
        ...
        // Free the image memory after using it.
