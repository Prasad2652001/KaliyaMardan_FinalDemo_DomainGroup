        ...
        // pushMatrix(modelMatrix);
        {
            ...
            for (const auto &row : bezierPoints)
            {
                ...
                if (index == vectorIndex)
                    glVertexAttrib4f(MATRIX_ATTRIBUTE_COLOR, 1.0, 1.0, 0.0, 1.0);
                else
                    glVertexAttrib4f(MATRIX_ATTRIBUTE_COLOR, 1.0, 0.0, 0.0, 1.0);

                glBindBuffer(GL_ARRAY_BUFFER, vbo_line_Position);
                glBufferData(GL_ARRAY_BUFFER, sizeof(float) * row.size(), row.data(), GL_DYNAMIC_DRAW);
                glBindBuffer(GL_ARRAY_BUFFER, NULL);

                glDrawArrays(GL_POINTS, 0, row.size() / 3);
                glBindVertexArray(NULL);
